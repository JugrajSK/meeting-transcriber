...
/** Nicely normalize common sharing links into direct-download audio URLs */
function normalizePublicUrl(url) {
  if (!url) return url;
  try {
    const u = new URL(url);

    // Google Drive: viewer/open → direct download
    // Formats:
    ...
    // OneDrive/SharePoint: add download=1 if missing
...
export default function AudioTranscriber() {
  const [file, setFile] = useState(null);
  const [audioUrlInput, setAudioUrlInput] = useState(''); // optional public URL
  const [status, setStatus] = useState('idle');
  const [transcript, setTranscript] = useState('');
  const [summary, setSummary] = useState('');
  const [error, setError] = useState('');
  ...
  // Direct-to-Blob (you can keep this; if the server route is fussy, use URL paste or your AAI upload route)
  async function uploadDirectToBlob(f) {
    const out = await blobUpload(f.name, f, {
      access: 'public',
  ...
  const handleProcess = async () => {
    try {
      ...
      // If user pasted a URL, normalize it; otherwise upload the file
      let audioUrl = (audioUrlInput || '').trim();
      if (audioUrl) {
        audioUrl = normalizePublicUrl(audioUrl);
      } else {
        if (!file) {
          setError('Please select a file or paste a public audio URL');
          return;
  ...
  async function useSampleAudio() {
    try {
      ...
      const sampleUrl = 'https://storage.googleapis.com/aai-web-samples/espn-basketball.wav';
      const text = await transcribeAudioUrl(sampleUrl);
      setTranscript(text);
      ...
      <div style={styles.mainCard}>
        ...
        <div style={styles.inputGroup}>
          ...
          <input
            ...
            onChange={(e) => setAudioUrlInput(e.target.value)}
            onPaste={(e) => {
              // auto-normalize on paste after the paste completes
              setTimeout(() => {
                setAudioUrlInput((prev) => normalizePublicUrl(prev));
              }, 0);
            }}
            placeholder="e.g., Google Drive viewer link (we'll convert it), Dropbox link (?dl=1), or https://.../file.mp3"
            style={styles.input}
          />
          <small style={{ color: '#718096' }}>
            Tip: Google Drive links are auto-converted to direct download. Dropbox links get <code>?dl=1</code>.
          </small>
        </div>
